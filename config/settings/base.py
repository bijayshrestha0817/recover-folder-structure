...
Environment-specific settings live alongside this module:
  - ``dev.py``  — local development (DEBUG on, relaxed).
  - ``prod.py`` — production hardening (HTTPS/HSTS/secure cookies, no insecure defaults).

``config/settings/__init__.py`` picks one based on the ``DJANGO_ENV`` env var
(default ``dev``), so ``DJANGO_SETTINGS_MODULE=config.settings`` keeps working.
...
# This file is config/settings/base.py, so the repo root is three parents up.
...
# The insecure default is for local/dev only — prod.py requires a real SECRET_KEY.
SECRET_KEY = env.str(
    "SECRET_KEY",
